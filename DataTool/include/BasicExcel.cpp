...
// REQUIRE: Block of data must be at least 512 bytes in size.
{
	...
	for (size_t i=0; i<109; ++i) LittleEndian::Write(block, BATArray_[(int)i], 0x004C+ (int)i*4, 4);
}
...
// REQUIRE: Block of data must be at least 512 bytes in size.
{
	...
	for (size_t i=0; i<109; ++i) LittleEndian::Read(block, BATArray_[(int)i], 0x004C+(int)i*4, 4);
	Initialize();		
}
...
// PROMISE: The file's original data will be replaced by the new data.
{
	...
	if (property->self_->size_ >= 4096)
	{
		if ((int)size >= 4096) property->self_->startBlock_ = (int)WriteData(data, size, property->self_->startBlock_, true);
		else
		{
			property->self_->startBlock_ = (int)WriteData(0, 0, property->self_->startBlock_, true);
			property->self_->startBlock_ = (int)WriteData(data, size, property->self_->startBlock_, false);
		}
	}
	else
	{
		if (size < 4096) property->self_->startBlock_ = (int)WriteData(data, size, property->self_->startBlock_, false);
		else
		{
			property->self_->startBlock_ = (int)WriteData(0, 0, property->self_->startBlock_, false);
			property->self_->startBlock_ = (int)WriteData(data, size, property->self_->startBlock_, true);
		}
	}
	property->self_->size_ = (int)size;
	SaveHeader();
	SaveBAT();
	SaveProperties();
	return SUCCESS;
...
// PROMISE: Changes will not be written to compound file.
{
	...
	{for (size_t i=0; i<109 && header_.BATArray_[i]!=-1; ++i)
	{
		...
		header_.BATArray_[(int)i] += (int)count;
	}}
	...
	header_.propertiesStart_ += (int)count;
	...
	if (!properties_.empty()) 
	{
		...
		properties_[0]->startBlock_ += (int)count;
	}
	{for (size_t i=1; i<maxProperties; ++i)
	{
		if (properties_[i]->size_ >= 4096)
		{
			...
			properties_[(int)i]->startBlock_ += (int)count;
		}
	}}
}
...
// PROMISE: Changes will not be written to compound file.
{
	...
	{for (size_t i=1; i<maxProperties; ++i)
	{
		if (properties_[i]->size_ >= 4096)
		{
			...
			properties_[i]->startBlock_ -= (int)count;
		}
	}}
}
...
// REQUIRE: parentpath and propertyname.
{
	...
	int npos;
	for (npos= (int)pathLength-1; npos>0; --npos)
	{
		if (path[npos] == L'\\') break; 
	}
...
// PURPOSE: Load all block allocation table information for compound file.
{
	...
	{for (size_t i=0; i<header_.BATCount_; ++i)
	{
		...
		{
			LittleEndian::Read(&*(block_.begin()), blocksIndices_[(int)j+ (int)i*128], (int)j*4, 4);
		}
	}}
	...
	{for (size_t i=0; i<header_.XBATCount_; ++i)
	{
		...
		for (size_t j=0; j<128; ++j) 
		{
			LittleEndian::Read(&*(block_.begin()), blocksIndices_[(int)(j+((i+109)*128))], (int)(j*4), 4);
		}
	}}
	...
	{for (size_t i=0; i<header_.SBATCount_; ++i)
	{
		...
		for (size_t j=0; j<128; ++j)
		{
			LittleEndian::Read(&*(block_.begin()), sblocksIndices_[(int)(j+i*128)], (int)(j*4), 4);
		}
	}}
}
...
// PURPOSE: Save all block allocation table information for compound file.
{
	...
	{for (size_t i=0; i<header_.BATCount_; ++i)
	{
		for (size_t j=0; j<128; ++j)
		{
			LittleEndian::Write(&*(block_.begin()), blocksIndices_[(int)(j+i*128)], (int)(j*4), 4);
		}
	...
	{for (size_t i=0; i<header_.XBATCount_; ++i)
	{
		for (size_t j=0; j<128; ++j)
		{
			LittleEndian::Write(&*(block_.begin()), blocksIndices_[(int)(j+((i+109)*128))], (int)(j*4), 4);
		}
	...
	{for (size_t i=0; i<header_.SBATCount_; ++i)
	{
		for (size_t j=0; j<128; ++j)
		{
			LittleEndian::Write(&*(block_.begin()), sblocksIndices_[(int)(j+i*128)], (int)(j*4), 4);
		}
...
// PROMISE: Returns the startIndex of new data for the property.
{
	if (isBig)
	{
		...
		int extraBlocks = (int)(maxPresentBlocks - maxNewBlocks);
		if (extraBlocks > 0)
		{
		...
		{			
			...
			do
			{
				...
				if (startIndex == -2) startIndex = (int)newIndex; // Get start index
		...
		if (extraSize != 0)
		{
			...
			if (curIndex >= maxPresentBlocks)
			{
				...
				if (startIndex == -2) startIndex = (int)newIndex;
	...
	else
	{
		...
		if (size != 0 && properties_[0]->startBlock_ == -2)
		{
			...
			properties_[0]->startBlock_ = (int)newIndex;
			properties_[0]->size_ = (int)header_.bigBlockSize_;
		}
	...
	else
	{
		...
		for (size_t i=0, c=0; i<uniqueStringsTotal_; ++i)
		{
			...
			else
			{
				...
				int size = (int)(continueIndices_[c] - npos - 1 - bytesRead);
				++c;
				if (size > 0) 
				{
...
BasicExcelWorksheet* BasicExcel::AddWorksheet(int sheetIndex)
{
	...
	{
		char sname[50];
		sprintf(sname, "Sheet%d", (int)(sheetNo++));
